{ pkgs, lib, pa, umport, makeUsers, recursiveMerge, ... }:
/*
let
in
*/
{ ############################ Variable scope ##################################
 ...
 #====<< User management >>====================================================>
  ...
  users.users = let
    t1 = makeUsers {
      userlist  = [ "hver" "admin" ]; 
      userrules = [ "wheel" "nixer" "networkmanager" ]; };
    t2 = makeUsers {
      userlist  = [ "guest" ]; 
      userrules = [ "guests" ]; };
 ...
 #====<< Network config >>=====================================================>
  networking = {
    hostName = "${pa.hostname}";     # The name of your computer.
